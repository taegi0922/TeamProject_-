											...
											<p>
												<c:choose>
													<c:when test="${commentsdto.writer eq dto.memberNum }">
														...
														<br/>
														<p>작성자: ${commentsdto.writerName } 작성일자: [<fmt:formatDate pattern="yyyy-MM-dd-HH:mm:ss" value="${commentsdto.created_at }"/>]</p>
														<textarea rows="5" cols="80" name="content" placeholder="${commentsdto.content }" required="required"></textarea>
						     			   				<button type="submit" id="btnCommentWrite"  class="myPre-btn">댓글 수정</button>
						     			   				<button type="button" class="myPre-btn"><a href='/mypage/prescriptDetail?prescript_no=<c:out value="${commentsdto.prescript_no }"/>&memberNum=<c:out value="${commentsdto.writer }"/>#modifyContent'>취소</a></button>
													</c:when>
													<c:otherwise>
														<h2>의사 Comment</h2>
