				...
				<div class="table-container">
					...
					<div class="card mb-4">
						<table id="boardTable" class="medicine-table">
							...
							<tbody>
								 <tr>
								<td colspan="6" class="no-rating">아직 남겨진 평점이 없습니다.</td>
								</tr>	
						...
						<div class="pageBtn">
							...
							<form id="pageForm" action="/mypage/myPrescriptRating" method="get">
		...
		$.ajax({
			...
			success: function(data) {
				...
				$.each(data, function(index, pr) {		
					...
					var pn = $("<td>").text(pr.prescript_no);
					row.append(pn);
					
					var patientName = $("<td>").text(pr.patient_name);
					row.append(patientName); 
					
					var patientPhone = $("<td>").text(pr.patient_phone);
					row.append(patientPhone);
					
					var parseDate = new Date(pr.prescribed_date); // DB에 저장된 등록 날짜를 Date객체로 변환
					...
					var rate = $("<td>").text(pr.rate);
					row.append(rate);
					
					/* <tr>
					<td data-th='No.'><c:out value="${PrescriptRating.prescriptdto.prescript_no}" /></td>
					<td data-th='환자명'><c:out value="${PrescriptRating.prescriptdto.patient_name}" /></td>
					<td data-th='환자 연락처'><c:out value="${PrescriptRating.prescriptdto.patient_phone}" /></td>
					<td data-th='날짜'><c:out value="${PrescriptRating.prescriptdto.prescribed_date}" /></td>
					<td data-th='평점'><c:out value="${PrescriptRating.ratingdto.rate}" /></td>
				<tr>	 */
